...
const char *device[] = {"SBUI", "SBUI", "SBUI"};
...
int initMYLCD_SBUI ()
{
	...
	mylog("starting sbui..\n");
	did = lSelectDevice(hw, device[0], "NULL", DWIDTH, DHEIGHT, DBPP, 0, &displays[0]);
	if (!did)
		did = lSelectDevice(hw, device[1], "NULL", DWIDTH, DHEIGHT, DBPP, 0, &displays[0]);
	if (!did)
		did = lSelectDevice(hw, device[2], "NULL", DWIDTH, DHEIGHT, DBPP, 0, &displays[0]);
